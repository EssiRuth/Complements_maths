...
# ------------------------------
#  Phase d’apprentissage du modèle Naïve Bayesien 
# ------------------------------

# Vérification de la taille des données d’entraînement et de la variable cible
dim(train_data)
head(target_train)

# Entraînement du modèle Naïve Bayésien avec les données d’entraînement
classif <- naive_bayes(x = train_data, y = target_train)

# Affichage du modèle entraîné pour s'assurer qu’il a bien été créé
classif
...
# Prédire les probabilités pour chaque classe (0 = décédé, 1 = survivant)
pred <- predict(classif, test_data, type = "prob")

# Afficher les premières lignes des probabilités prédites
head(pred)

# Prédire la classe (0 ou 1) pour chaque observation du test
target_pred <- predict(classif, test_data, type = "class")

# Vérifier que la longueur des prédictions correspond à celle des vraies valeurs
stopifnot(length(target_pred) == length(target_test))

# ------------------------------
# Évaluation des performances du modèle
# ------------------------------

# Création de la courbe ROC à partir des probabilités de la classe 1 (survivant)
roc_obj <- roc(response = target_test, predictor = pred[, "1"])

# Tracer la courbe ROC
plot(roc_obj, main = "Courbe ROC - Naïve Bayésien", col = "blue", lwd = 2)

# Calcul de l’AUC (aire sous la courbe) pour mesurer la qualité globale du modèle
auc_value <- auc(roc_obj)
print(paste("AUC: ", round(auc_value, 4)))

# Générer la matrice de confusion pour voir où le modèle se trompe
conf_matrix <- confusionMatrix(target_pred, target_test)
print(conf_matrix)

# Calcul du taux de bonnes prédictions (Accuracy)
accuracy <- mean(target_pred == target_test)

# ------------------------------
#  Calcul du rappel et de la précision pour la classe "1" (survivant)
# ------------------------------

# S'assurer que les deux vecteurs sont bien des facteurs avec les bons niveaux
target_pred_adjusted <- factor(target_pred, levels = c("0", "1"))
target_test <- factor(target_test, levels = c("0", "1"))

# Calcul du rappel (Recall) : capacité à détecter les survivants
recall_adjusted <- recall(target_pred_adjusted, target_test, relevant = "1")

# Calcul de la précision (Precision) : fiabilité des prédictions de survivants
