...
# III. Les méthodes de prédiction

## 1. La méthode des k plus proches voisins avec partage du jeu de données en Holdout

### 1.1. Transformation des variables explicatives
...
### 1.2. Les variables explicatives et partage des données
...
#### Partition du Jeu de Données en Train/Validation/Test
...
# 10% pour la validation (hors test)
notinTest <- setdiff(1:nobs, inTest)
inValidation <- sample(notinTest, size = round(0.1 * nobs), replace = FALSE)

# 80% restant pour train (hors test et validation)
inTrain <- setdiff(1:nobs, union(inTest, inValidation))
...
### 1.3. Sélection de l’Hyperparamètre k
...
### 1.4. Les différentes métriques en fonction des valeurs de K étudiées
...
### 1.5. Performance du classifieur final
...
#### Prédiction de la target et la matrice de confusion
...
recall_test    <- cm$byClass["Sensitivity"]  # Recall
...
#### Utilisation de la partie Test pour évaluer le classifieur final

```{r}

# Prédiction sur les données vues pendant l’apprentissage
target_pred = knn(
  train = rbind(predictive_vars_train, predictive_vars_validation),
  test  = rbind(predictive_vars_train, predictive_vars_validation),  # ⚠️ test = train ici
  cl    = c(target_train, target_validation),
  k     = 5, prob = TRUE
)

# Matrice de confusion
cm <- confusionMatrix(target_pred, c(target_train, target_validation), positive = "1")

# Métriques
accuracy_train  = cm$overall["Accuracy"]
recall_train    = cm$byClass["Sensitivity"]
precision_train = cm$byClass["Precision"]
