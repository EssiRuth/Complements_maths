...
library(ggplot2)
```
Lien de téléchargement de la base de données : https://www.kaggle.com/datasets/iamsouravbanerjee/heart-attack-prediction-dataset
...
## 1.2 Méthode de cross-validation et calcul des moyennes sur les 10 Folds
...
-   Sensitivity (Recall) : 8.76 %, Ce faible score indique que le modèle
    ne détecte qu’une petite partie des cas positifs (patients à
    risque). Autrement dit, il y a un grand nombre de faux négatifs.
    
-   Precision : 34.6 %, Cela signifie qu’environ 34% des prédictions
    positives sont correctes.

####  Élagage de l'arbre
Nous allons donc procéder à l’élagage de l’arbre de décision pour réduire sa complexité et améliorer sa capacité de généralisation.

```{r, eval = FALSE}
classifTree$cptable
```

Nous commençons par examiner la table de complexité (cptable) de l'arbre et sélectionnons la valeur optimale de cp correspondant à l'erreur minimale. Nous affichons ensuite la table des valeurs de cp et traçons le graphique de l’erreur en fonction de cp pour identifier le point optimal :
```{r, eval = FALSE}
...
L’arbre est ensuite élagué en multipliant cp par 0.8, une approche courante pour éviter un élagage excessif. Il est visualisé ci-dessous. 
Cette représentation graphique permet d’identifier les variables les plus
...
de test pour calculer les métriques
...
-   **Précision** : 44.30 %. Lorsqu’un patient est identifié comme étant
    à risque, il y a seulement 44% de chances qu’il le soit
    réellement.
...
> ne parvient à détecter qu’environ **9 % des patients réellement à
...
prédits comme étant à risque, **environ 34% le sont effectivement**.
...
# Résultats fictifs à adapter avec tes vraies valeurs si besoin
...
kable(resultats_modeles, caption = "Tableau comparatif des performances des différents modèles", booktabs = TRUE)
```

## Interprétation
Le tableau ci-dessus met en évidence des performances contrastées entre les différents modèles testés :
    - k-NN (k = 4) est celui qui offre le meilleur compromis entre accuracy, capacité à détecter les vrais positifs et précision. Il apparaît donc comme le modèle le plus performant dans ce contexte.
    
    - La régression logistique et le modèle Naïve Bayes présentent une accuracy correcte, mais échouent complètement à détecter les patients à risque, les rendant inadaptés pour une application médicale sans ajustement.
    
    - Les arbres de décision affichent une performance globale modérée, mais leur faible sensibilité (moins de 9 %) limite leur capacité à identifier les cas positifs, malgré une meilleure précision.
    
Ces résultats soulignent un déséquilibre de classe dans les données, pénalisant les modèles conservateurs. Ils appellent à l’intégration de techniques de rééquilibrage (comme SMOTE) et/ou de modèles plus robustes comme les forêts aléatoires ou les modèles ensemblistes pour améliorer la détection des patients à risque.

# III. Bilan de l'étude

Dans le cadre de ce projet académique, nous avons cherché à concevoir un modèle de classification permettant de prédire le risque d’infarctus du myocarde à partir d’un jeu de données cliniques et comportementales. Pour cela, nous avons exploré et comparé plusieurs approches supervisées, notamment les k-plus proches voisins, la régression logistique, les arbres de décision et le classifieur naïf bayésien.
Nos expérimentations ont montré que le modèle k-NN avec k = 4 a fourni les résultats les plus équilibrés en termes de performance. Il atteint une accuracy d’environ 69 %, avec une capacité de détection des cas à risque (recall) de 46 % et une précision de 59 %. Cela signifie qu’il réussit à identifier presque un patient à risque sur deux, tout en limitant les fausses alertes.
En revanche, certains modèles, comme la régression logistique et le naïf bayesien, bien que simples et généralement robustes, n’ont pas permis de détecter efficacement les cas à risque (recall = 0 % dans notre cas), ce qui les rend inadaptés dans une logique de prévention médicale. Les arbres de décision, même après élagage, ont présenté des performances limitées, notamment une sensibilité très faible, ce qui est problématique dans un contexte où les faux négatifs doivent être évités autant que possible.
Nous avons également observé que certaines variables habituellement associées au risque cardiovasculaire, telles que le cholestérol ou l’indice de masse corporelle, n’avaient qu’un pouvoir prédictif modéré dans notre étude. Cela pourrait être dû aux interactions complexes entre variables, ou à des limites propres à notre jeu de données.

Ce projet nous a permis de :
    Mieux comprendre les spécificités du domaine médical, où la performance d’un modèle ne se résume pas à l’accuracy mais repose avant tout sur sa capacité à identifier correctement les individus à risque.
    Approfondir nos compétences en préparation de données, validation croisée et interprétation des métriques de classification.
    Réaliser à quel point le déséquilibre des classes peut affecter un modèle, et à quel point il est important d’adapter les méthodes en conséquence.

# IV. Recommandations
Notre travail reste perfectible, et plusieurs pistes pourraient être explorées dans la suite :
    Mettre en œuvre des techniques de rééquilibrage comme SMOTE pour améliorer la détection des cas à risque.
    Tester des modèles plus performants comme les forêts aléatoires ou le XGBoost, qui sont souvent mieux adaptés aux problématiques de santé.
    Repenser les seuils de classification, voire utiliser des métriques plus adaptées comme le F1-score ou l’AUC-ROC, pour une évaluation plus complète.

# Conclusion
Ce projet nous a permis de confronter nos connaissances théoriques à un cas réel, en mettant en pratique différentes méthodes de classification sur un problème médical concret. Bien que les résultats obtenus soient encore modestes, cette étude constitue une première étape encourageante vers la construction de systèmes prédictifs au service de la santé publique.

# Répartition du travail 

Ce projet a été réalisé de manière collaborative, chaque membre de l’équipe ayant apporté une contribution spécifique tout en participant activement aux différentes étapes du travail.  

La première partie, consacrée à la préparation du jeu de données et à l’analyse univariée, a été rédigée par KOSSI Essi Ruth. Elle s’est chargée du nettoyage des données, de la gestion des valeurs manquantes et de la normalisation, tout en explorant les distributions des variables pour mieux comprendre leur impact potentiel sur la prédiction du risque de crise cardiaque.  

DESSI Akou Bernadette a pris en charge les analyses multivariées ainsi que la mise en œuvre des algorithmes des k-plus-proches voisins. Elle a expérimenté différentes stratégies de validation, notamment la méthode du holdout et la validation croisée, afin d’optimiser le choix de k. Elle s’est également occupée de l’implémentation et de l’analyse des arbres de décision, en ajustant les paramètres pour améliorer les performances du modèle.  

Yasmina sur les méthodes de classification basées sur la régression logistique et le classifieur naïf bayésien. Elle a étudié la pertinence de ces modèles dans le contexte du projet, comparé leurs performances avec celles des autres algorithmes et cherché à améliorer leur précision par des ajustements adaptés.  

Bien que chaque membre ait eu une responsabilité principale, le travail s’est déroulé dans un esprit d’entraide et d’échange constant. Lorsqu’un blocage survenait, les autres intervenaient pour proposer des solutions, tester des alternatives ou approfondir certaines analyses. Cette approche collaborative a permis d’optimiser l’efficacité du projet et de renforcer notre compréhension collective des techniques utilisées.
