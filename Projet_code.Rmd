...
# Entraînement du modèle Naïve Bayes (ou régression logistique, selon ce que vous voulez)
logit_model <- glm(target_train ~ ., 
                   data = train_data, 
                   family = binomial(link = "logit"))

# Prédiction des probabilités avec régression logistique
target_pred_prob <- predict(logit_model, newdata = test_data, type = "response")

# Ajustement du seuil (ici à 0.3 pour plus de sensibilité à la classe 1)
target_pred_adjusted <- ifelse(target_pred_prob > 0.3, 1, 0)

# Vérification des prédictions ajustées
table(target_pred_adjusted)

# Calcul des métriques de performance

# Convertir les prédictions et la cible en facteur
target_pred_adjusted <- factor(target_pred_adjusted, levels = levels(target_test))
...
print(paste("Adjusted Accuracy: ", conf_matrix$overall['Accuracy']))
print(paste("Adjusted Recall (survivant): ", conf_matrix$byClass['Recall']))
print(paste("Adjusted Precision (survivant): ", conf_matrix$byClass['Precision']))
```
